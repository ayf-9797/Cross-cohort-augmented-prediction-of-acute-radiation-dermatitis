...
## 0. 安装并加载所需包
##############################
needed_pkgs <- c("caret", "naivebayes", "MLmetrics", "e1071", "ggplot2", 
...
##############################
nb_grid <- expand.grid(laplace = c(0, 0.5, 1),
                       usekernel = c(TRUE, FALSE),
                       adjust = c(0.5, 1, 1.5))

nb_model <- train(psoriasis ~ .,
                  data = patient_df,
                  method = "naive_bayes",
                  trControl = ctrl,
                  tuneGrid = nb_grid,
...
# 提取交叉验证结果
cv_results <- nb_model$resample
mean_F1 <- mean(cv_results$F1)
best_F1 <- max(cv_results$F1)

cat("模型训练完成\n")
cat("最佳参数: laplace =", nb_model$bestTune$laplace, 
...
# 提取最佳参数下的预测结果
best_params <- nb_model$bestTune
pred_best <- nb_model$pred %>%
  filter(laplace == best_params$laplace,
         usekernel == best_params$usekernel,
         adjust == best_params$adjust)
...
p_roc <- ggplot(roc_plot_data, aes(x = fpr, y = tpr, color = fold_label)) +
  ...
  coord_equal() +
  labs(
    title = "NB - 5-Fold Cross-Validation ROC Curves",
...
print(p_roc)
ggsave("ROC_5-Fold_NB.png", p_roc, width = 10, height = 7, dpi = 300)
...
p_prc <- ggplot(prc_plot_data, aes(x = recall, y = precision, color = fold_label)) +
  ...
  labs(
    title = "NB - 5-Fold Cross-Validation PR Curves",
...
print(p_prc)
ggsave("PRC_5-Fold_NB.png", p_prc, width = 10, height = 7, dpi = 300)
...
ggsave("ROC_5-Fold_NB_with_average.png", p_roc_with_avg, width = 8, height = 7, dpi = 300)
...
p_fold_performance <- ggplot(performance_long, aes(x = metric, y = value)) +
  ...
  labs(
    title = "NB 5-Fold Cross-Validation Performance Metrics",
...
ggsave("fold_performance_metrics_NB.png", p_fold_performance, width = 8, height = 6, dpi = 300)
saveRDS(performance_long,file="performance_nb.rds")
##############################
## 8. 保存模型与结果
##############################

# 保存模型
saveRDS(nb_model, file = "NB_best_model.rds")
...
saveRDS(NB_roc_data, file = "NB_roc_data.rds")
...
saveRDS(validation_results, file = "NB_validation_results.rds")

cat("\n🟢 模型已保存为 NB_best_model.rds\n")
cat("🟢 ROC/PRC数据已保存为 NB_roc_data.rds\n")
cat("🟢 验证结果已保存为 NB_validation_results.rds\n")
...
saveRDS(dca_NB_df, "dca_NB_df.rds")
cat("🟢 决策曲线数据已保存为 dca_NB_df.rds\n")
...
cat("   - 最优参数: sigma =", nb_model$bestTune$sigma, ", C =", nb_model$bestTune$C, "\n")
...
cat("\n4. 输出文件:\n")
cat("   - 模型文件: NB_best_model.rds\n")
cat("   - ROC/PRC数据: NB_roc_data.rds\n")
cat("   - 验证结果: NB_validation_results.rds\n")
cat("   - 决策曲线数据: dca_NB_df.rds\n")
cat("   - 图形文件:\n")
cat("     * ROC_5-Fold_NB.png\n")
cat("     * PRC_5-Fold_NB.png\n")
cat("     * ROC_5-Fold_NB_with_average.png\n")
cat("     * fold_performance_metrics_NB.png\n")
...
p_roc <- ggplot(roc_plot_data, aes(x = fpr, y = tpr, color = fold_label)) +
  ...
  coord_equal() +
  labs(
    title = "Naive Beyas - 5-Fold Cross-Validation ROC Curves",
    subtitle = sprintf("Mean AUROC = %.3f (95%% CI: %.3f - %.3f)", 
                       mean_auroc, ci_auroc[1], ci_auroc[2]),
    x = "False Positive Rate (1 - Specificity)",
    y = "True Positive Rate (Sensitivity)",
...
p_prc <- ggplot(prc_plot_data, aes(x = recall, y = precision, color = fold_label)) +
  ...
  annotate("text", x = 0.5, y = baseline_precision - 0.03, 
           ...
           color = "grey50", size = 3.5) +
  labs(
    title = "Naive Beyas - 5-Fold Cross-Validation PR Curves",
    subtitle = sprintf("Mean AUPRC = %.3f (95%% CI: %.3f - %.3f)", 
                       mean_auprc, ci_auprc[1], ci_auprc[2]),
    x = "Recall (Sensitivity)",
    y = "Precision",
...
saveRDS(sub_nb_roc_data, file = "sub_nb_roc_data.rds")
}
...
ggsave("sub_threshold_peformance_nb.png", p_fold_performance_sub, width = 10, height = 7, dpi = 300)
saveRDS(sub_performance_long,file="sub_performance_nb.rds")
}
### 全量数据模型的新数据验证
if (NEWDATA){
  ...
  new_pred <- predict(nb_model, new_data, type = "prob")
  if (BALANCE_SAMPLE){
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_nb.xlsx", sheetName = "2",col.names=TRUE, row.names=FALSE, append=TRUE)
  }else{
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_nb.xlsx", sheetName = "1",col.names=TRUE, row.names=FALSE, append=TRUE)
  }
