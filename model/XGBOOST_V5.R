...
needed_pkgs <- c("caret", "MLmetrics", "e1071", "ggplot2", "purrr", 
                 "dplyr", "tidyr", "readxl", "magrittr", "ROCR","xlsx", "xgboost")
...
# 测试模型
# patient_df <- read_excel('../data/large_model0801/impulate_0801_control.xlsx')
...
patient_df$psoriasis <- factor(patient_df$psoriasis, levels = c("0", "1"), labels = c("No", "Yes"))
...
## 4. 训练XGBoost模型 (替换SVM)
##############################
# caret的xgbTree支持分类，先制作onehot矩阵在送入，根据label自动确定分类模型
...
x <- model.matrix(psoriasis ~ ., data = patient_df)[,-1]
y = patient_df$psoriasis
xgb_model <- train(
  x=x,
  y=y,
  method = "xgbDART",
...
xgb_imp_df <- data.frame(
  Variable = rownames(xgb_importance$importance),
  Importance = xgb_importance$importance$Overall
)

# 合并子分层
xgb_imp_summary <- merge_factor_importance(xgb_imp_df, patient_df)

# 打印结果
print("xgb特征重要性排名（合并后）：")
print(xgb_imp_summary)

# 绘制随机森林重要性图
topN <- min(10, nrow(xgb_imp_summary))
p_xgb <- ggplot(xgb_imp_summary[1:topN, ], 
               aes(x = reorder(Variable, Importance), y = Importance)) +
  geom_col(fill = "#2ca02c", alpha = 0.8) +
  coord_flip() +
  labs(
    title = sprintf("XGBoost Variable Importance (Top %d)", topN),
    x = NULL,
...
print(p_xgb)
ggsave("xgb_variable_importance.png", p_xgb, width = 8, height = 6, dpi = 300)
saveRDS(xgb_imp_summary, "xgboost_var_important.rds")
##############################
## 5. 提取评估结果并输出
##############################
...
calib_data_xgb <- pred_best %>%
  select(prob = Yes, truth = obs) %>%
  mutate(model = "XGBoost")

# 保存文件 - RDS 供多个模型汇总使用
saveRDS(calib_data_xgb, file = "calibration_data_xgboost.rds")
##############################
## 6. ROC和PRC曲线绘制
##############################
...
p_roc <- ggplot(roc_plot_data, aes(x = fpr, y = tpr, color = fold_label)) +
  ...
  coord_equal() +
  labs(
    title = "XGBoost - 5-Fold Cross-Validation ROC Curves",
...
print(p_roc)
ggsave("ROC_5-Fold_XGBoost.png", p_roc, width = 10, height = 7, dpi = 300)
...
p_prc <- ggplot(prc_plot_data, aes(x = recall, y = precision, color = fold_label)) +
  ...
  labs(
    title = "XGBoost - 5-Fold Cross-Validation PR Curves",
...
print(p_prc)
ggsave("PRC_5-Fold_XGBoost.png", p_prc, width = 10, height = 7, dpi = 300)
...
ggsave("ROC_5-Fold_XGBoost_with_average.png", p_roc_with_avg, width = 8, height = 7, dpi = 300)
...
p_fold_performance <- ggplot(performance_long, aes(x = metric, y = value)) +
  ...
  labs(
    title = "XGBoost 5-Fold Cross-Validation Performance Metrics",
  ...
  ylim(0, 1)
saveRDS(performance_long,file="performance_xgboost.rds")
print(p_fold_performance)
ggsave("fold_performance_metrics_xgboost.png", p_fold_performance, width = 8, height = 6, dpi = 300)

##############################
## 8. 保存模型与结果
##############################
saveRDS(xgb_model, file = "xgboost_best_model.rds")
xgb_roc_data <- list(
...
saveRDS(xgb_roc_data, file = "xgboost_roc_data.rds")
validation_results <- list(
  fold_performance = fold_performance,
  performance_df = performance_df,
  avg_performance = avg_performance
)
saveRDS(validation_results, file = "xgboost_validation_results.rds")

cat("\n🟢 模型已保存为 xgboost_best_model.rds\n")
cat("🟢 ROC/PRC数据已保存为 xgboost_roc_data.rds\n")
cat("🟢 验证结果已保存为 xgboost_validation_results.rds\n")

##############################
## 9. 决策曲线分析（可选）
##############################
dca_xgb_df <- pred_best %>%
  select(prob = Yes, truth = obs) %>%
  mutate(
    model = "XGBoost",
    truth = ifelse(truth == "Yes", 1, 0)
  )
saveRDS(dca_xgb_df, "dca_xgboost_df.rds")
cat("🟢 决策曲线数据已保存为 dca_xgboost_df.rds\n")

##############################
## 10. 模型性能总结报告
##############################
cat("\n", rep("=", 60), "\n", sep = "")
cat("XGBoost 模型五折交叉验证性能总结\n")
cat(rep("=", 60), "\n", sep = "")

cat("\n1. 模型参数:\n")
cat("   - 方法: XGBoost (xgbTree)\n")
cat("   - 最优参数:\n")
print(xgb_model$bestTune)
...
cat("\n4. 输出文件:\n")
cat("   - 模型文件: xgboost_best_model.rds\n")
cat("   - ROC/PRC数据: xgboost_roc_data.rds\n")
cat("   - 验证结果: xgboost_validation_results.rds\n")
cat("   - 决策曲线数据: dca_xgboost_df.rds\n")
cat("   - 图形文件:\n")
cat("     * ROC_5-Fold_XGBoost.png\n")
cat("     * PRC_5-Fold_XGBoost.png\n")
cat("     * ROC_5-Fold_XGBoost_with_average.png\n")
cat("     * fold_performance_metrics_xgboost.png\n")
...
print(p_roc)
ggsave("ROC_5-Fold_xgboost_sub.png", p_roc, width = 10, height = 7, dpi = 300)
...
print(p_prc)
ggsave("PRC_5-Fold_xgboost_sub.png", p_prc, width = 10, height = 7, dpi = 300)
sub_xgboost_roc_data <- list(
  roc_data = all_roc_data,
  prc_data = all_prc_data,
  auc_values = auc_values,
  prauc_values = prauc_values,
...
saveRDS(sub_xgboost_roc_data, file = "sub_xgboost_roc_data.rds")
# 提取校准曲线
pred_best_subgroup <- pred_best %>%
  filter(patient_df[rowIndex, ]$Diagnosis == 2)

calib_data_sub_xgb <- pred_best_subgroup %>%
  select(prob = Yes, truth = obs) %>%
  mutate(
    model = "XGBoost",
...
saveRDS(calib_data_sub_xgb, "calibration_data_xgboost_subgroup.rds")
}
# 如果还要计算分层指标
...
if (SUB_THRESHOLD){
  ...
  ggsave("sub_threshold_peformance_xgboost.png", p_fold_performance_sub, width = 10, height = 7, dpi = 300)
  saveRDS(sub_performance_long,file="sub_performance_xgboost.rds")
  saveRDS(sub_result_df, file="sub_threshold_xgb.rds")
}
...
if (NEWDATA){
  ...
  new_data$psoriasis <- factor(new_data$psoriasis, 
                               levels = c("0", "1"), 
                               labels = c("No", "Yes"))
  temp_ <- rbind(patient_df, new_data)
  temp_m <- model.matrix(psoriasis ~ ., data = temp_)[, -1]
  a = nrow(patient_df)+1
  b = nrow(patient_df)+nrow(new_data)
  new_input <- temp_m[a:b,]
  new_pred <- predict(xgb_model, new_input, type = "prob")
  if (BALANCE_SAMPLE){
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_xgb.xlsx", sheetName = "2",col.names=TRUE, row.names=FALSE, append=TRUE)
  }else{
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_xgb.xlsx", sheetName = "1",col.names=TRUE, row.names=FALSE, append=TRUE)
  }
