...
cat("模型训练完成\n")
cat("最佳参数: sigma =", svm_model$bestTune$sigma, ", C =", svm_model$bestTune$C, "\n")
...
ggsave("ROC_5-Fold_SVM_with_average.png", p_roc_with_avg, width = 8, height = 7, dpi = 300)
...
p_fold_performance <- ggplot(performance_long, aes(x = metric, y = value)) +
  ...
  labs(
    title = "SVM 5-Fold Cross-Validation Performance Metrics",
  ...
  ylim(0, 1)
saveRDS(performance_long,file="performance_svm.rds")
print(p_fold_performance)
ggsave("fold_performance_metrics_svm.png", p_fold_performance, width = 8, height = 6, dpi = 300)
...
saveRDS(validation_results, file = "svm_validation_results.rds")

cat("\n🟢 模型已保存为 svm_best_model.rds\n")
cat("🟢 ROC/PRC数据已保存为 svm_roc_data.rds\n")
cat("🟢 验证结果已保存为 svm_validation_results.rds\n")
...
cat("🟢 决策曲线数据已保存为 dca_svm_df.rds\n")

##############################
## 10. 模型性能总结报告
##############################
cat("\n", rep("=", 60), "\n", sep = "")
cat("SVM 模型五折交叉验证性能总结\n")
cat(rep("=", 60), "\n", sep = "")

cat("\n1. 模型参数:\n")
cat("   - 方法: Support Vector Machine (Radial Kernel)\n")
cat("   - 最优参数: sigma =", svm_model$bestTune$sigma, ", C =", svm_model$bestTune$C, "\n")
...
cat("\n4. 输出文件:\n")
cat("   - 模型文件: svm_best_model.rds\n")
cat("   - ROC/PRC数据: svm_roc_data.rds\n")
cat("   - 验证结果: svm_validation_results.rds\n")
cat("   - 决策曲线数据: dca_svm_df.rds\n")
cat("   - 图形文件:\n")
cat("     * ROC_5-Fold_SVM.png\n")
cat("     * PRC_5-Fold_SVM.png\n")
cat("     * ROC_5-Fold_SVM_with_average.png\n")
cat("     * fold_performance_metrics_svm.png\n")
...
saveRDS(sub_svm_roc_data, file = "sub_svm_roc_data.rds")
}
...
if (SUB_THRESHOLD){
  ...
  ggsave("sub_threshold_peformance_svm.png", p_fold_performance_sub, width = 10, height = 7, dpi = 300)
  saveRDS(sub_performance_long,file="sub_performance_svm.rds")
...
if (NEWDATA){
  ...
  new_pred <- predict(svm_model, new_input, type = "prob")
  if (BALANCE_SAMPLE){
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_svm.xlsx", sheetName = "2",col.names=TRUE, row.names=FALSE, append=TRUE)
  }else{
    write.xlsx(new_pred$Yes, "../data/select_data_model0806/new_svm.xlsx", sheetName = "1",col.names=TRUE, row.names=FALSE, append=TRUE)
  }
